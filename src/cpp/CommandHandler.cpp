...
bool CommandHandler::executeCommand(std::string& text, int clientId) {
    std::vector<std::string> parts = this->parseText(text);

    if (parts[0] == "/say" && parts.size() >= 2) {
        returnText = "";
    ...
    if (parts[0] == "/setspawn" && parts.size() == 1) {
    ...
    if (parts[0] == "/op" && parts.size() == 2) {
        this->privateText = true;
        if (server->adminSystem->isAdmin(clientId)) {
